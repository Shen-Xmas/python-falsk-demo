...
__all__ = ["ConcurrentRotatingFileHandler", "MultiprocessRotatingLog"]
...
class MultiprocessRotatingLog(object):
    """
    支持多进程，多线程的RotatingFileHandler
    """
    level_relations = {
        'debug': logging.DEBUG,
        'info': logging.INFO,
    ...
    def __init__(self, project_path: str = None, service_name: str = None, log_file: str = None, mode: str = "a"):
        """
        构造方法
        :param project_path:   项目根路径
        :param service_name:    # 模块文件启动，service_name 必须是 __name__
                                # 使用 flask 启动，service_name 必须是 werkzeug
                                # 使用 gunicorn 启动，service_name 必须是 gunicorn.error
        ...
        # 如果日志目录不存在，则创建
        self.logs_dir = Path(project_path, "logs")
        if not self.logs_dir.is_dir():
            os.makedirs(self.logs_dir)

        self.service_name = service_name if service_name else __name__
        log_file = Path(self.logs_dir, log_file) if log_file else None
        self.file_log = log_file if log_file else Path(self.logs_dir,
                                                       "{}.log".format(os.path.basename(sys.argv[0]).split(".")[0]))
        self.console_formatter = self.file_formatter = logging.Formatter(
            "%(asctime)s - %(filename)s - [PID-%(process)d] - [Line-%(lineno)d] - " +
            "[Thread-%(thread)d] - [%(levelname)s] - %(message)s - <%(funcName)s>")
        self.root_level = self.file_level = self.console_handler = self.level_relations.get("info")
        ...
        logging_config = getattr(ProjectConfig.get_object(), "logging")
        if logging_config:
            root = getattr(logging_config, "root")
            console = getattr(logging_config, "console")
            file = getattr(logging_config, "file")
            if root:
                self.root_handler = root.root_handler
                self.root_level = self.level_relations.get(root.root_level)
                self.encoding = root.encoding
                self.stream = eval(root.stream)
            if console:
                self.console_handler = console.console_handler
                self.console_formatter = logging.Formatter(console.console_formatter)
                self.console_level = self.level_relations.get(console.console_level)
                self.stdout = console.stdout
            if file:
                self.file_log = log_file if log_file else Path(self.logs_dir, file.file_log)
                self.file_handler = file.file_handler
                self.file_formatter = logging.Formatter(file.file_formatter)
                self.file_level = self.level_relations.get(file.file_level)
                self.max_bytes = file.max_bytes
                self.back_count = file.back_count
        self.__log = logging.getLogger(self.service_name)
        self.__log.setLevel(self.root_level)
        """
        实例化RotatingFileHandler
        backCount: 是备份文件个数，如果超过这个个数，就会自动覆盖
        maxBytes: 为每个日志文件大小
        delay: 是否开启OutStream缓存
                   True 表示开启缓存，OutStream输出到缓存，待缓存区满后，刷新缓存区，并输出缓存数据到文件。
                   False 表示不开启缓存，OutStream直接输出到文件
        use_zip: 为True时，自动gzip分切的日志
        """
        file_handler = ConcurrentRotatingFileHandler(self.file_log)
        file_handler.encoding = self.encoding
        file_handler.mode = mode
        file_handler.maxBytes = self.max_bytes
        file_handler.backupCount = self.back_count
        file_handler.use_gzip = False
        file_handler.delay = False
        file_handler.debug = False
        file_handler.setFormatter(self.file_formatter)
        file_handler.setLevel(self.file_level)
        file_handler.name = "file_rotate"
        if file_handler.name not in [x.name for x in self.__log.handlers]:
            self.__log.addHandler(file_handler)

        # 设置日志要不要在控制台输出
        if self.stdout:
            console_handler = logging.StreamHandler()
            console_handler.stream = self.stream
            console_handler.name = "console"
            console_handler.setFormatter(self.console_formatter)
            console_handler.setLevel(self.console_level)
            if console_handler.name not in [x.name for x in self.__log.handlers]:
